...
sed -i "s/html_errors = .*/html_errors = On/" /etc/php/7.0/cli/php.ini
...
sed -i "s/DocumentRoot .*/DocumentRoot \\/vagrant\\/public" /etc/apache2/sites-available/000-default.config
...
if [ -e /vagrant/data/seed-mysql.sql ]; then
	mysql -u root marketing_info < /vagrant/data/seed-mysql.sql
fi

# allow anyone to connect to mysql - not for production!
...
# Map up and down to history search once a command has been started
